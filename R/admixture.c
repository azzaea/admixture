...
// input u = u1 + u2, where (u1,u2) is the phased genotype. Inputs
// x and u may be either vectors or matrices.
double genoprob (double x, double u, double e);
...
// -----------------------------------------------------------------
// This is the initial implementation of the function above,
// admixture_unlabeled_Estep_Call. It is less efficient than the
// function above, but it is easier to understand, so I am keeping it
// around for posterity.
SEXP admixture_unlabeled_Estep_Call_v1 (SEXP Xp, SEXP Fp, SEXP Qp, SEXP ep, 
					SEXP mp, SEXP n0p, SEXP n1p, SEXP r00p,
					SEXP r01p, SEXP r10p, SEXP r11p) {
  ...
  // calculations below.
  double x;           // A genotype.
  double f1, f2;      // Allele frequencies.
  double q1, q2, q12; // Admixture proportions.  
  double w;           // Normalizing constant for posterior probabilities.
  double r;
  ...
  for (i = 0; i < n; i++)
	...
	for (k2 = 0; k2 < K; k2++) {
	  ...
	  // Get the admixture proportions for population k1 and k2 in
	  // sample i.
	  t   = k1 + K*k2;
	  q1  = Q[i + n*k1];
	  q2  = Q[i + n*k2];
	  ...
	  // Get the genotype for sample i at marker j (x), the
	  // allele frequency of marker j in population k1 (f1), and
	  // the allele frequency of marker j in population k2 (f2).
	  x  = X[i + n*j];
	  f1 = F[j + p*k1];
	  f2 = F[j + p*k2];
      ...
      // Add the posterior probabilities to the expected population
      // counts and expected allele counts.
      for (k1 = 0; k1 < K; k1++)
      	for (k2 = 0; k2 < K; k2++) {
	  t             = k1 + K*k2;
	  t1            = k1 + K*k2;
	  t2            = k2 + K*k1;
	  r             = (r00[t] + r01[t] + r10[t] + r11[t])/w;
	  m[i + n*k1]  += r;
	  m[i + n*k2]  += r;
	  n0[j + p*k1] += (r00[t1] + r01[t1] + r00[t2] + r10[t2])/w;
	  n1[j + p*k1] += (r10[t1] + r11[t1] + r01[t2] + r11[t2])/w;
	}
    }
